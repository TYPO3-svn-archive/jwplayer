...
(function (b) {
        ...
        if (b.utils.hasFlash()) {
            if (d) {
                ...
                if (d.playlist && d.playlist.length) {
                    f.file = d.playlist[0].file;
                    f.levels = d.playlist[0].levels
                } else {
                    f.file = d.file;
                    f.levels = d.levels
                }
                if (typeof f.file == "undefined" && typeof f.levels == "undefined") {
                    ...
                    if (f.file) {
                        return b.utils.flashCanPlay(f.file)
                    } else {
                        if (f.levels && f.levels.length) {
                            for (var e = 0; e < f.levels.length; e++) {
                                if (f.levels[e].file && b.utils.flashCanPlay(f.levels[e].file)) {
                                    return true
                                }
                            }
                        }
    ...
    b.utils.browserSupportsConfig = function (e) {
        var d = document.createElement("video");
        if ( !! d.canPlayType) {
            if (e) {
                var g = {};
                if (e.playlist && e.playlist.length) {
                    g.file = e.playlist[0].file;
                    g.levels = e.playlist[0].levels
                } else {
                    g.file = e.file;
                    g.levels = e.levels
                }
                if (typeof g.file == "undefined" && typeof g.levels == "undefined") {
                    ...
                    if (g.file) {
                        return b.utils.browserCanPlay(d, g.file)
                    } else {
                        if (g.levels && g.levels.length) {
                            for (var f = 0; f < g.levels.length; f++) {
                                if (g.levels[f].file && b.utils.browserCanPlay(d, g.levels[f].file)) {
                                    return true
                                }
                            }
                        }
    ...
    b.utils.browserCanPlay = function (e, d) {
        var f = b.utils.extension(d);
        if (b.utils.extensionmap[f] !== undefined && b.utils.extensionmap[f].html5 !== undefined) {
            sourceType = b.utils.extensionmap[f].html5
        } else {
            sourceType = "video/" + f + ";"
        }
        if (b.utils.isLegacyAndroid() && f.match(/m4v|mp4/)) {
            return true
        }
        return (e.canPlayType(sourceType) || d.toLowerCase().indexOf("youtube.com") > -1)
    };
    b.utils.flashCanPlay = function (e) {
        var d = false;
        var f = b.utils.extension(e);
        if (b.utils.extensionmap[f] !== undefined && b.utils.extensionmap[f].flash !== undefined) {
            d = b.utils.extensionmap[f].flash
        }
        return (d || e.toLowerCase().indexOf("youtube.com") > -1)
    };
...
(function (b) {
    ...
    b.api.addPlayer = function (c) {
    	/**
    	 * ATTENTION (bugfix/workaround for AJAX-requests):
    	 * do not add player to list, otherwise we will get problems if we load some jwplayer via AJAX
    	 */
/*
            ...
            if (e && e.type) {
                ...
                case "flash":
                    if (a.utils.flashSupportsConfig(this.config)) {
                        ...
                        var g = a.embed.embedFlash(document.getElementById(this.api.id), e, this.config);
                        this.api.container = g;
                    ...
                    break;
                case "html5":
                    if (a.utils.browserSupportsConfig(this.config)) {
                        var f = a.embed.embedHTML5(document.getElementById(this.api.id), e, this.config);
                        this.api.container = document.getElementById(this.api.id);
                        this.api.setPlayer(f)
                    } else {
                        this.players.splice(0, 1);
                        return this.embedPlayer()
                    }
    ...
    a.html5.mediavideo = function (f, D) {
        ...
        function J(V) {
            ...
            for (var P = 0; P < V.levels.length; P++) {
                ...
                if (a.utils.browserCanPlay(Q, O.file)) {
...
(function (a) {
    a.html5.api = function (b, k) {
        var j = {};
        if (!a.utils.browserSupportsConfig()) {
            return j
        }
